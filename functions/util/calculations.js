module.exports = {
    ...
    dps: (damage, time, digits) => {
        const precision = Math.pow(10, digits);
        return time === 0 ? 'N/A' : Math.round(damage / time * precision) / precision;
    },
    ttk: (kos, time, digits) => {
        const precision = Math.pow(10, digits);
    ...
    suicide_rate: (suicides, games, digits) => {
        const precision = Math.pow(10, digits);
        return suicides === 0 ? 'N/A' : Math.round(games / suicides * precision) / precision;
    },
    ...
    avg_kos_per_game: (kos, games, digits) => {
        const precision = Math.pow(10, digits);
        return games === 0 ? 'N/A' : Math.round(kos / games * precision) / precision;
    }
}