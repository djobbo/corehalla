	...
	return new Promise((resolve, reject) => {
		var player = {
			...
			global: {
				...
				winrate:
					Math.round((playerStats.wins / playerStats.games) * 1000) /
					10
			},
			season: {
				...
				winrate:
					Math.round((playerRank.wins / playerRank.games) * 1000) / 10
			},
		...
		//Teams
		if (playerRank['2v2']) {
			player.teams = parse2v2TeamsStats(
				player.id,
				player.name,
				playerRank['2v2']
			);
		...
		for (var i = 0; i < legends_data.length; i++) {
			...
			if (legendStats) {
				...
				legend.matchtime_format = moment
					.duration(legendStats.matchtime, 'seconds')
					.format('hh[h] mm[m] ss[s]');
				legend.damage =
					parseInt(legendStats.damageweaponone) +
					parseInt(legendStats.damageweapontwo) +
					parseInt(legendStats.damageunarmed) +
					parseInt(legendStats.kogadgets) +
					parseInt(legendStats.kothrownitem);
				legend.dps =
					Math.round((legend.damage / legend.matchtime) * 10) / 10;
				legend.ttk =
					Math.round((legend.matchtime / legend.kos) * 10) / 10;
				legend.ttf =
					Math.round((legend.matchtime / legend.falls) * 10) / 10;
				legend.global = {
					...
					winrate:
						Math.round(
							(legendStats.wins / legendStats.games) * 1000
						) / 10
				};
				legend.suicide_rate =
					Math.round((legend.global.games / legend.suicides) * 10) /
					10;
				legend.game_length =
					Math.round((legend.matchtime / legend.global.games) * 10) /
					10;
				legend.kos_per_game =
					Math.round((legend.kos / legend.global.games) * 10) / 10;
				legend.falls_per_game =
					Math.round((legend.falls / legend.global.games) * 10) / 10;
				legend.weapons = {
					weapon_one: {
						name: weapon_one,
						time_held: legendStats.timeheldweaponone,
						time_held_format: moment
							.duration(legendStats.timeheldweaponone, 'seconds')
							.format('hh[h] mm[m] ss[s]'),
					...
					weapon_two: {
						...
						time_held_format: moment
							.duration(legendStats.timeheldweapontwo, 'seconds')
							.format('hh[h] mm[m] ss[s]'),
				...
				player.unarmed.damage += parseInt(
					legend.weapons.unarmed.damage
				);
				player.unarmed.kos += legend.weapons.unarmed.kos;

				player.weapons[weapon_one].time_held +=
					legend.weapons.weapon_one.time_held;
				player.weapons[weapon_one].global.games += legend.global.games;
				player.weapons[weapon_one].global.wins += legend.global.wins;
				player.weapons[weapon_one].global.losses +=
					legend.global.losses;
				player.weapons[weapon_one].damage += parseInt(
					legend.weapons.weapon_one.damage
				);
				player.weapons[weapon_one].kos += legend.weapons.weapon_one.kos;

				player.weapons[weapon_two].time_held +=
					legend.weapons.weapon_two.time_held;
				player.weapons[weapon_two].global.games += legend.global.games;
				player.weapons[weapon_two].global.wins += legend.global.wins;
				player.weapons[weapon_two].global.losses +=
					legend.global.losses;
				player.weapons[weapon_two].damage += parseInt(
					legend.weapons.weapon_two.damage
				);
			...
			if (legendRank) {
				legend.season = {
					...
					winrate:
						Math.round(
							(legendRank.wins / legendRank.games) * 1000
						) / 10
				};

				player.weapons[weapon_one].season.games += legendRank.games;
				player.weapons[weapon_one].season.wins += legendRank.wins;
				player.weapons[weapon_one].season.losses +=
					legendRank.games - legendRank.wins;

				if (
					player.weapons[weapon_one].season.highest_rating <
					legendRank.rating
				) {
					player.weapons[weapon_one].season.highest_rating =
						legendRank.rating;
					player.weapons[
						weapon_one
					].season.highest_rating_legend = legend_name_bio;
				} else if (
					player.weapons[weapon_one].season.highest_rating ===
					legendRank.rating
				) {
					player.weapons[weapon_one].season.highest_rating_legend +=
						' • ' + legend_name_bio;
				}

				if (
					player.weapons[weapon_one].season.highest_peak_rating <
					legendRank.rating
				) {
					player.weapons[weapon_one].season.highest_peak_rating =
						legendRank.rating;
					player.weapons[
						weapon_one
					].season.highest_peak_rating_legend = legend_name_bio;
				} else if (
					player.weapons[weapon_one].season.highest_peak_rating ===
					legendRank.rating
				) {
					player.weapons[
						weapon_one
					].season.highest_peak_rating_legend +=
						' • ' + legend_name_bio;
				}

				player.weapons[weapon_two].season.games += legendRank.games;
				player.weapons[weapon_two].season.wins += legendRank.wins;
				player.weapons[weapon_two].season.losses +=
					legendRank.games - legendRank.wins;

				if (
					player.weapons[weapon_two].season.highest_rating <
					legendRank.rating
				) {
					player.weapons[weapon_two].season.highest_rating =
						legendRank.rating;
					...
					player.weapons[weapon_two].season.highest_rating_legend +=
						' • ' + legend_name_bio;
				}

				if (
					player.weapons[weapon_two].season.highest_peak_rating <
					legendRank.rating
				) {
					player.weapons[weapon_two].season.highest_peak_rating =
						legendRank.rating;
		...
		player.legends.sort(function(a, b) {
			return (
				b.xp - a.xp ||
				b.season.rating - a.season.rating ||
				b.season.peak_rating - a.season.peak_rating ||
				b.name.localeCompare(a.name)
			);
		});
		...
		player.weapons.sort(function(a, b) {
			return (
				b.time_held - a.time_held ||
				b.global.games - a.global.games ||
				b.global.wins - a.global.wins ||
				b.name.localeCompare(a.name)
			);
		});

		for (var i = 0; i < player.weapons.length; i++) {
			player.weapons[i].time_held_format = moment
				.duration(player.weapons[i].time_held, 'seconds')
				.format('hh[h] mm[m] ss[s]');
			player.weapons[i].global.winrate =
				player.weapons[i].global.games === 0
					? 'N/A'
					: Math.round(
							(player.weapons[i].global.wins /
								player.weapons[i].global.games) *
								1000
					  ) / 10;
			player.weapons[i].season.winrate =
				player.weapons[i].season.games === 0
					? 'N/A'
					: Math.round(
							(player.weapons[i].season.wins /
								player.weapons[i].season.games) *
								1000
					  ) / 10;
			if (player.weapons[i].time_held === 0) {
				...
				player.weapons[i].dps =
					Math.round(
						(player.weapons[i].damage /
							player.weapons[i].time_held) *
							10
					) / 10;
				player.weapons[i].ttk =
					Math.round(
						(player.weapons[i].time_held / player.weapons[i].kos) *
							10
					) / 10;
			}
		}

		player.unarmed.time_held_format = moment
			.duration(player.unarmed.time_held, 'seconds')
			.format('hh[h] mm[m] ss[s]');
		player.matchtime_format = moment
			.duration(player.matchtime, 'seconds')
			.format('hh[h] mm[m] ss[s]');
		...
		player.suicide_rate =
			Math.round((player.global.games / player.suicides) * 10) / 10;
		player.game_length =
			Math.round((player.matchtime / player.global.games) * 10) / 10;
		player.kos_per_game =
			Math.round((player.kos / player.global.games) * 10) / 10;
		player.falls_per_game =
			Math.round((player.falls / player.global.games) * 10) / 10;
...
function parseClanStats(clanStats) {
	return new Promise((resolve, reject) => {
		for (var i = 0; i < clanStats.clan.length; i++) {
			...
			clanStats.clan[i].join_date = moment
				.unix(clanStats.clan[i].join_date)
				.utc()
				.format('MMMM DD, YYYY');
		}
		...
		var clan = {
			...
			create_date: moment
				.unix(clanStats.clan_create_date)
				.utc()
				.format('MMMM DD, YYYY'),
...
function parse2v2TeamsStats(player_id, player_name, teamsStats) {
	...
	for (var i = 0; i < teamsStats.length; i++) {
		...
		parsedTeams.push({
			...
			player_two_id:
				teamsStats[i].brawlhalla_id_one === player_id
					? teamsStats[i].brawlhalla_id_two
					: teamsStats[i].brawlhalla_id_one,
			player_two_name:
				teamsStats[i].brawlhalla_id_one === player_id
					? teamname.substring(teamname.indexOf('+') + 1)
					: teamname.substring(0, teamname.indexOf('+')),
			...
			winrate:
				Math.round((teamsStats[i].wins / teamsStats[i].games) * 1000) /
				10
		});
	}
	return parsedTeams;
