...
function TreeNode({ children, title, onClick }: PropsWithChildren<{ title: string; onClick?: () => void }>) {
    ...
    return (
        <>
            <p className={styles.treeNode} onClick={() => (onClick ? onClick() : setIsVisible((v) => !v))}>
                <svg
            ...
            <TreeNode title="Level">
                <TreeNode title="Animations">
                    {mapData.animations.map((anim) => (
                        <TreeNode title={`PlatID ${anim.platId}`} key={anim.platId}>
                            {anim.keyframes.map((key) =>
                                key.type === 'Keyframe' ? (
                                    <TreeNode title={`Key ${key.frameNum}`} key={key.frameNum} />
                                ) : (
                                    <TreeNode title={`Phase ${key.frameNum}`}>
                                        {key.keyFrames.map((k) => (
                                            <TreeNode title={`Key {k.frameNum}`} key={k.frameNum} />
                                        ))}
                                    </TreeNode>
                                ),
                            )}
                ...
                <TreeNode title="Dynamic Collisions">
                    {mapData.dynamicCollisions.map((dynCol) => (
                        <TreeNode title={`PlatID ${dynCol.platId}`} key={dynCol.platId}>
                ...
                <TreeNode title="Platforms">
                    {mapData.platforms.map((plat) => (
                        <TreeNode title={`PlatID ${plat.id}`} key={plat.platId}>
                            {plat.platforms.map((childPlat) => (
                                <TreeNode title={childPlat.id} key={childPlat.platId} /> //TODO: ++ recursion
                            ))}
                        </TreeNode>
                    ))}
                </TreeNode>
                <TreeNode title="Moving Platforms">
                    {mapData.movingPlatforms.map((movPlat) => (
                        <TreeNode title={`PlatID ${movPlat.id}`} key={movPlat.id}>
