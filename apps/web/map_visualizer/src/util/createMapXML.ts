export function createMapXML(mapData: LevelDesc): string {
    ...
    mapData.collisions.forEach((col) => {
        outStr += `    <${col.type}Collision ${col.x1 === col.x2 ? `X="${col.x1}"` : `X1="${col.x1}" X2="${col.x2}"`} ${
            col.y1 === col.y2 ? `Y="${col.y1}"` : `Y1="${col.y1}" Y2="${col.y2}"`
        }/>\n`
    })
