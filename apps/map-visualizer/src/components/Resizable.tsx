...
function ResizeAnchor({
    ...
    return (
        <div
            className={`${styles.anchor} ${isDragging ? styles.isDragging : ''}`}
...
export function Resizable({ children, widthThreshold, minWidth, maxWidth }: Props): JSX.Element {
    ...
    return (
        <>
            {cloneElement(
                children,
                {
                    className: `${children.props.className || ''} ${styles.container}`,
                    ref: resizableRef,
                },
                [
